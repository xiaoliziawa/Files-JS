...
import net.prizowo.filesjs.FilesJSPlugin;
import net.prizowo.filesjs.Filesjs;
...
public class FilesWrapper {
    ...
    public void writeFile(String path, String content) {
        try {
            ...
            if (isNewFile) {
                ...
                FileEventJS event = new FileEventJS(path, content, "changed", null, server, level);
                FilesJSPlugin.FILE_CHANGED.post(ScriptType.SERVER, event);
            }
        } catch (IOException e) {
    ...
    public void createDirectory(String path) {
        try {
            ...
            MinecraftServer server = ServerLifecycleHooks.getCurrentServer();
            ServerLevel level = server.overworld();
            FileEventJS event = new FileEventJS(path, null, "directory_created", null, server, level);
            FilesJSPlugin.DIRECTORY_CREATED.post(ScriptType.SERVER, event);
        } catch (IOException e) {
    ...
    public void delete(String path) {
        try {
            ...
            if (isDirectory) {
                FilesJSPlugin.DIRECTORY_DELETED.post(ScriptType.SERVER, event);
            } else {
                FilesJSPlugin.FILE_DELETED.post(ScriptType.SERVER, event);
            }
        } catch (IOException e) {
    ...
    public void copy(String source, String target) {
        try {
            ...
            MinecraftServer server = ServerLifecycleHooks.getCurrentServer();
            ServerLevel level = server.overworld();
            FileEventJS event = new FileEventJS(target, null, "copied", null, server, level);
            FilesJSPlugin.FILE_COPIED.post(ScriptType.SERVER, event);
        } catch (IOException e) {
    ...
    public void move(String source, String target) {
        try {
            ...
            FileEventJS event = new FileEventJS(target, content, "moved", null, server, level);
            FilesJSPlugin.FILE_MOVED.post(ScriptType.SERVER, event);
        } catch (IOException e) {
    ...
    public void mergeFiles(List<String> sourcePaths, String targetPath) {
        try {
            ...
            FilesJSPlugin.FILES_MERGED.post(ScriptType.SERVER, event);
        } catch (IOException e) {
        ...
        try {
            ...
            if (watchService != null) {
                try {
                    ...
                    MinecraftServer server = ServerLifecycleHooks.getCurrentServer();
                    ServerLevel level = server.overworld();
                    FileEventJS event = new FileEventJS(path, null, "watch_stopped", null, server, level);
                    FilesJSPlugin.FILE_WATCH_STOPPED.post(ScriptType.SERVER, event);
                } catch (IOException e) {
                ...
                try {
                    if (changedPath.getFileName().equals(fileName) && Files.exists(changedPath)) {
                        ...
                        if (1.0 - similarity > threshold) {
                            ...
                            FilesJSPlugin.FILE_CONTENT_CHANGED_SIGNIFICANTLY.post(ScriptType.SERVER, event);
                        }
                    }
                } catch (IOException e) {
	...
	private void filescopy(Path sourcePath, Path backupPath) throws IOException {
		...
		FilesJSPlugin.FILE_BACKUP_CREATED.post(ScriptType.SERVER, event);
	}
    ...
    public void renameFile(String oldPath, String newPath) {
        try {
            ...
            MinecraftServer server = ServerLifecycleHooks.getCurrentServer();
            ServerLevel level = server.overworld();
            FileEventJS event = new FileEventJS(newPath, content, "renamed", null, server, level);
            FilesJSPlugin.FILE_RENAMED.post(ScriptType.SERVER, event);
        } catch (IOException e) {
